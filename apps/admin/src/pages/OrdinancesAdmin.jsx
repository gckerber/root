// apps/admin/src/pages/OrdinancesAdmin.jsx
...
const CATEGORIES = ['budget', 'zoning', 'general', 'traffic', 'utilities', 'police']
...
function OrdForm({ item, onSave, onCancel, adminKey }) {
  ...
  async function handleSave() {
    if (!form.number?.trim() || !form.title?.trim()) {
      toast('Number and title are required', 'error')
      return
    }
    setUploading(true)
    try {
    ...
    <div className="card p-5 border-orange-600/20 bg-orange-600/5 space-y-3">
      <div className="grid sm:grid-cols-2 gap-3">
        <div>
          ...
          <input className="input font-mono" value={form.number} onChange={f('number')}
      ...
      <div>
        <label className="label">
          PDF File {item?.fileUrl && '(re-upload to replace)'}
        </label>
          ...
          >
            {file ? (
              <span className="text-white flex items-center gap-2">
                <Paperclip size={14} className="text-orange-400" />{file.name}
              </span>
            ) : (
              <span className="text-slate-500 flex items-center gap-2">
                <Upload size={14} /> Click to attach PDF
              </span>
            )}
          </div>
          {item?.fileUrl && !file && (
            <a href={item.fileUrl} target="_blank" rel="noreferrer" className="btn-ghost py-2 px-3">
              <Download size={13} />
            </a>
          )}
        </div>
...
export default function OrdinancesAdmin() {
  ...
  async function handleSave(form) {
    ...
    try {
      ...
      setItems((prev) =>
        isEdit ? prev.map((x) => x.id === saved.id ? saved : x) : [saved, ...prev]
      )
      setAdding(false)
      setEditingId(null)
  ...
  async function handleDelete(item) {
    ...
    try {
      await fetch(`${API}/api/ordinances?id=${item.id}&category=${item.category}`, {
        method: 'DELETE',
    ...
    <div className="max-w-3xl space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <select className="input py-2 w-36" value={filterCat}
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center text-slate-600">No ordinances found.</div>
      ) : (
        filtered.map((item) =>
          editingId === item.id ? (
            <OrdForm key={item.id} item={item} onSave={handleSave}
              onCancel={() => setEditingId(null)} adminKey={auth.key} />
          ) : (
            <div key={item.id} className="card p-4 flex items-center gap-4">
              ...
              <div className="flex-grow min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="badge bg-orange-600/20 text-orange-400">{item.category}</span>
                  <span className="text-slate-600 text-xs">{item.year}</span>
                </div>
              ...
              <div className="flex gap-2 flex-shrink-0">
                {item.fileUrl && (
                  <a href={item.fileUrl} target="_blank" rel="noreferrer" className="btn-ghost py-1.5 px-3">
                    <Download size={13} />
                  </a>
                )}
                <button onClick={() => setEditingId(item.id)} className="btn-ghost py-1.5 px-3">
                  <Pencil size={13} />
                </button>
