...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const items = search
        ? resources.filter(m => m.title?.toLowerCase().includes(search) || m.type?.toLowerCase().includes(search))
        : resources
    ...
    if (req.method === 'POST') {
      ...
      const record = {
        ...
        year,                          // partition key
        title: `${date.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })} Council Meeting`,
    ...
    if (req.method === 'DELETE') {
      ...
      await container.item(id, parseInt(year)).delete()  // year is a number partition key
