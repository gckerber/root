// apps/admin/src/pages/BulletinAdmin.jsx
...
// Removed 'general' category as requested
const CATEGORIES = ['notice', 'event', 'urgent']
const CAT_COLORS = {
    ...
    <div className="card p-5 border-yellow-600/20 bg-yellow-600/5 space-y-3">
      ...
      <div>
        ...
        <input className="input" value={form.title} onChange={f('title')}
          placeholder="e.g. Road Closure — Elm Street" />
      </div>
      <div>
        <label className="label">Message</label>
        <textarea className="input resize-none" rows={4} value={form.body} onChange={f('body')}
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input className="input" type="url" value={form.link || ''} onChange={f('link')}
            placeholder="https://..." />
        </div>
      </div>
      <label className="flex items-center gap-3 cursor-pointer">
        ...
        <span className="text-slate-300 text-sm flex items-center gap-1.5">
          <Pin size={13} className="text-yellow-400" /> Pin to top of bulletin board
        </span>
      </label>
      <div className="flex gap-2 pt-1">
        <button onClick={() => onSave(form)} className="btn-primary">
          <Save size={14} /> {item ? 'Update' : 'Post'} Announcement
        </button>
    ...
    <div className="max-w-3xl space-y-4">
            ...
            {['all', ...CATEGORIES].map((c) => (
              <button key={c} onClick={() => setFilterCat(c)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors capitalize ${
                  filterCat === c ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-slate-300'
                }`}>
                {c}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <div className="card p-10 text-center text-slate-600">
          No announcements yet. Click "Post Announcement" to add one.
        </div>
      ) : (
        filtered.map((item) =>
          editingId === item.id ? (
              ...
              <div className="flex items-start gap-3">
                <div className="flex-grow min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className={`badge ${CAT_COLORS[item.category] || 'bg-slate-600/20 text-slate-400'}`}>
                      {item.category}
                    </span>
